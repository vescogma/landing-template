...
function fetcher({ endpoint = BASE_URL, config = {}, headers = {}, json = true }) {
  const extendedConfig = (0, _extends3.default)({}, DEFAULT_PARAMS, params);
  extendedConfig.headers = (0, _extends3.default)({}, DEFAULT_HEADERS, headers);
...
// modular rest calls
function get({ pieces = [], query = {}, base = true, config = {} }) {
  const endpoint = createEndpoint({ pieces, base, query });
  config.method = 'get';
  return fetcher({ endpoint, config });
}

function post({ pieces = [], body = {}, base = true, config = {} }) {
  const endpoint = createEndpoint({ pieces, base });
  config.method = 'post';
  config.body = (0, _stringify2.default)(body);
  return fetcher({ endpoint, config });
}

function put({ pieces = [], body = {}, base = true, config = {} }) {
  const endpoint = createEndpoint({ pieces, base });
  config.method = 'put';
  config.body = (0, _stringify2.default)(body);
  return fetcher({ endpoint, config });
...
function destroy({ pieces = [], body = {}, base = true, config = {} }) {
  const endpoint = createEndpoint({ pieces, base });
...
function download(pieces = [], query = {}, base = true, config = {}) {
  const endpoint = createEndpoint({ pieces, base, query });
  return fetcher({ endpoint, config, json: false }).then(response => response.blob()).then(blob => {
